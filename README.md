  ...
  1. C7 — run uv lock (lockfile is stale)                                                                                                       
  2. C3 — swap AHashMap → BTreeMap at materialization for deterministic word ordering
  3. C4 — sort local_pos_updates before iterating
  4. C1 — replace unsafe PyO3 0.23 iterator idioms with PyO3 0.27 safe API (try_iter() / unbind())
  5. C2 — remove dead if current <= 0 { break; } or replace with a reachable equivalent
  6. C8 — add rustbpe/.gitignore
